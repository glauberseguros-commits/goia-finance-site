...
def test_parser_nfe_xml():
    dados = DocumentoNFeController.ler(XML_NFE)

    assert dados["numero"] == "12345"
    assert dados["serie"] == "1"
    assert dados["emitente"]["cnpj"] == "28860122000177"
    assert dados["destinatario"]["cnpj"] == "11222333000181"
    assert dados["valor_total"] == 200.0
    assert len(dados["itens"]) == 1
    assert dados["itens"][0]["descricao"] == "Produto Teste"

    print("OK - Parser NF-e XML válido.")
